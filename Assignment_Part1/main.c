...
int thread_priority[]={30,30,30,30,50,51,51,20};  /* P1, P2, P3, P4, D, AP1, AP2, Rx */
struct sched_param param[8];
int rerror[8]; /* to check for error in pthread creation */
int i;
const char thread_name[8][15]={"Periodic1","Periodic2","Periodic3","Periodic4","Daemon","APeriodic1","APeriodic2","Receiver"};
...
const int* rid[1] = {&r_id[0]}; /* pointer to receiver thread */
	...
	/* Create Data Queues 1 and 2 */
	data_queue1 = sq_create();
	data_queue2 = sq_create();
	...
	pthread_setname_np(thread_ID[i],thread_name[i]);	
	if(rerror[i] != 0)
		{
	...
	pthread_setname_np(thread_ID[i],"Daemon");
	if(rerror[4] != 0)
		{
	...
	pthread_setname_np(thread_ID[i],thread_name[7]);
	if(rerror[7] != 0)
		{
	...
	sq_delete(data_queue1);
	sq_delete(data_queue2);
	exit(0);
...
void* Periodic_Threadfunc(void *ptr){
	...
	while(!Terminate_Flag) /* While termination sequence is not initiated */
	{
		...
		if((*source_id == 0) || (*source_id == 1)) /* Depending on the thread no, buffer is selected */
			Err = sq_write(data_queue1,Msg);  
		else
			Err = sq_write(data_queue2,Msg);
